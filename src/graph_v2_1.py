...
# GOAL
Generate 3-5 strategic follow-up questions that will help clarify the research scope, gather missing context, and ensure the research meets the user's specific needs.

# OUTPUT FORMAT
Generate 3-5 strategic questions in numbered format:
...
4. [Strategic question about constraints/limitations]
5. [Strategic question about desired outcomes/usage]
...
def execute_sequential_workflow(initial_state, config):
    """
    Execute HITL graph first, then main graph with the results.
    
    This function implements the sequential workflow execution pattern:
    1. First runs the Human-in-the-Loop (HITL) workflow to generate research queries
    2. Then runs the main research workflow starting with retrieve_rag_documents
    
    The state handover between workflows ensures that:
    - research_queries generated in HITL are passed to the main workflow
    - user_query, detected_language, and other context is preserved
    - LLM model selections are maintained across workflows
    
    Args:
        initial_state (dict): Initial state containing user_query and other parameters
        config (RunnableConfig): Configuration for the workflow execution
        
    Yields:
        dict: Step information with step name, data, and type for progress tracking
    """
    # Convert ResearcherStateV2 to InitState for HITL
    hitl_initial_state = InitState(
        user_query=initial_state["user_query"],
        current_position=initial_state.get("current_position", 0),
        detected_language=initial_state.get("detected_language", "English"),
        additional_context=initial_state.get("additional_context", ""),
        human_feedback=initial_state.get("human_feedback", ""),
        analysis="",
        follow_up_questions="",
        report_llm=initial_state.get("report_llm", "deepseek-r1:latest"),
        summarization_llm=initial_state.get("summarization_llm", "deepseek-r1:latest")
    )
    
    # Execute HITL workflow
    hitl_result = None
    for step in hitl_graph.stream(hitl_initial_state, config=config):
        step_name = list(step.keys())[0] if step else "unknown"
        step_data = step.get(step_name, {}) if step else {}
        
        yield {
            "step": f"hitl_{step_name}",
            "data": step_data,
            "type": "hitl_step"
        }
        
        if step_name == "__end__":
            hitl_result = step_data
            break
    
    if not hitl_result:
        raise Exception("HITL workflow did not complete successfully")
    
    yield {
        "step": "hitl_complete",
        "data": {"message": "Human-in-the-loop phase completed. Starting main research workflow..."},
        "type": "transition"
    }
    
    # Prepare state for main workflow
    main_initial_state = ResearcherStateV2(
        user_query=hitl_result.get("user_query", initial_state["user_query"]),
        current_position=0,
        detected_language=hitl_result.get("detected_language", "English"),
        research_queries=hitl_result.get("research_queries", []),
        retrieved_documents={},
        search_summaries={},
        final_answer="",
        quality_check=None,
        additional_context=hitl_result.get("additional_context", ""),
        report_llm=hitl_result.get("report_llm", "deepseek-r1:latest"),
        summarization_llm=hitl_result.get("summarization_llm", "deepseek-r1:latest"),
        query_mapping=None,
        enable_quality_checker=config["configurable"].get("enable_quality_checker", False),
        human_feedback=initial_state.get("human_feedback", "")
    )
    
    # State handover from HITL to main workflow is complete
    # Main workflow will start with retrieve_rag_documents using research_queries from HITL
    
    # Execute main workflow
    for step in main_graph.stream(main_initial_state, config=config):
        step_name = list(step.keys())[0] if step else "unknown"
        step_data = step.get(step_name, {}) if step else {}
        
        yield {
            "step": f"main_{step_name}",
            "data": step_data,
            "type": "main_step"
        }
        
        if step_name == "__end__":
            final_answer = step_data.get("final_answer", "No final answer generated.")
            yield {
                "step": "final_answer",
...
researcher_v2 = execute_sequential_workflow
